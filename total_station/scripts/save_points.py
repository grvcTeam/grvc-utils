...
def save_waypoints():
    # Parse arguments
    parser = argparse.ArgumentParser(description='Save points to a yaml file')
    parser.add_argument('-plan_package', type=str, default='total_station',
                        help='Name of the package where points will be stored')
    ...
    file_name = raw_input("Enter saved points file name (leave empty to use default): ")
    if file_name is '':
        file_name = 'points.yaml'
    ...
    rospy.loginfo('All points in frame_id: %s', frame_id)
    file_url = plans_dir + file_name
    yaml_file = open(file_url, 'w')
    yaml_file.write('frame_id: ' + frame_id + '\n')
    rospy.loginfo('Points will be saved to: %s', file_url)
    ...
    for wp_id in range(1000):
        ...
        if current_frame_id != frame_id:
            rospy.logerr("Point NOT SAVED: Current frame_id [%s] differs from expected [%s]", current_frame_id, frame_id)
            continue
        rospy.loginfo('Saving current pose as %s', wp_to_save)
        yaml_file.write(wp_to_save + '\n')

    if wp_id > 0:
        rospy.loginfo('All points saved to: %s', file_url)
    yaml_file.close()
    return
